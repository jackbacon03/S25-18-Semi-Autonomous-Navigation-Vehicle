...
void handleUserInput() {
  ...
  if (SerialUIF.available() > 0) {
    ...
    } else if (input.startsWith("I=")) {  // If User Changes I-Constant
      Ki = constrain(input.substring(2).toFloat(), 0.0, 5.0);
      SerialUIF.print("ACK: I-Constant set to ");
      SerialUIF.println(Ki, 2);
...
*/
void readSensors(bool IS_FRONT) {     // Determine if we are reading from FRONT or BACK sensor
